...
- **Data source (AID-09 — full-DB):** the **Prisma + Postgres database (Neon) is the source of truth**. All data reads go through `getEntries()` in `src/lib/db.ts`, which queries the DB when `DATABASE_URL` is set and falls back to `public/images.json` otherwise (local dev / DB down). New centers are added/edited via the admin UI (`/admin-db-<slug>/`, session-gated API under `src/pages/api/admin/`), not by hand. The image → `image-metadata.json` → `images.json` pipeline (below) still runs at build and remains the seed/fallback; to re-sync the DB from JSON, run `DATABASE_URL=... node scripts/seed-db.mjs`. Secrets (`DATABASE_URL` pooled, `SESSION_SECRET`) live in `.env` locally (gitignored) and in Netlify env vars.
- **Admin auth (AID-013):** the admin API is account-based — no shared token. Access requires a logged-in account whose `role` is `admin`. Seed the first admin with `npm run seed-users` (creates greg / password 1234). `SESSION_SECRET` (a random string) signs the HMAC session cookies (`aid_session`). Auth endpoints: `POST /api/auth/signup`, `POST /api/auth/login`, `POST /api/auth/logout`, `GET /api/auth/me`. Admin routes require role `admin`; `POST /api/submissions` requires any authenticated user.
