...
// (the browser can't read the folder page directly because of CORS). The
// "Ingresados" tab calls this on each reload; new files in the Drive appear
// without rebuilding. Falls back to the static public/data/ingresados.json.
...
const ORDER: Record<string, number> = { consolidado: 0, hospital: 1, busqueda: 2, otro: 3 };

export const GET: APIRoute = async () => {
  let items: { name: string; url: string; kind: string }[] = [];
  try {
    ...
    /* folder unreachable → empty; client falls back to the static file */
  }
